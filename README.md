...
To integration `ghasum` as a local action into your GitHub Actions workflows you
have to first create the local action and then use it in every job in every
workflow.
...
2. <details>
   ...
   jobs:
     example:
       ...
       #  action is ran before the checksums are verified it should be pinned to
       #  a commit SHA.
